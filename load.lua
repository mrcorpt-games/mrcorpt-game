...
if getgenv().NexoLoaded and hui:FindFirstChild("NexoKeySystem") then return getgenv().Nexo end
if getgenv().NexoLoaded and hui:FindFirstChild("NexoKeylessSystem") then return getgenv().Nexo end
getgenv().NexoLoaded = true
...
local function getFileName()
    return FolderName .. "/" .. Nexo.Storage.FileName .. ".txt"
end

local function saveKey(key)
    if not fileSystemSupported or not Nexo.Storage.Remember then return false end
...
local function getLogoIcon()
    if Nexo.Appearance.Icon == DefaultLogoAsset then return getIcon("logo") end
    return Nexo.Appearance.Icon
end

local function shouldDownloadLogo()
    return Nexo.Appearance.Icon == DefaultLogoAsset
end

local function getShopIcon()
    if Nexo.Shop.Icon == "" then return getLogoIcon() end
    return Nexo.Shop.Icon
end

local function isShopEnabled()
    return Nexo.Shop.Enabled
end
...
local function enableBlur()
    if not Nexo.Options.Blur then return end
    local existing = Lighting:FindFirstChild("NexoKeySystemBlur")
    if existing then existing:Destroy() end
    Internal.BlurEffect = Instance.new("BlurEffect")
    Internal.BlurEffect.Name = "NexoKeySystemBlur"
...
local function fullCleanup()
    getgenv().NexoLoaded = false
    getgenv().NexoClosed = true
    disableBlur()
    local gui1 = hui:FindFirstChild("NexoKeySystem")
    local gui2 = hui:FindFirstChild("NexoKeylessSystem")
    local gui3 = hui:FindFirstChild("NexoLoadingScreen")
...
local function setupDragging(header, main)
    if not Nexo.Options.Draggable then return end
...
local function CreateDoorOverlay(parentFrame, width, height)
    ...
    leftDoor.BackgroundColor3 = Nexo.Theme.Header
    ...
    rightDoor.BackgroundColor3 = Nexo.Theme.Header
    ...
    logoImage.ImageColor3 = Nexo.Theme.Text
...
local function ShowLoadingScreen(onComplete)
    ...
    local oldGui = hui:FindFirstChild("NexoLoadingScreen")
    if oldGui then oldGui:Destroy() end
    local oldBlur = Lighting:FindFirstChild("NexoLoadingBlur")
    if oldBlur then oldBlur:Destroy() end

    local blurEffect = Instance.new("BlurEffect")
    blurEffect.Name = "NexoLoadingBlur"
    blurEffect.Size = 0
    blurEffect.Parent = Lighting

    local gui = Instance.new("ScreenGui")
    gui.Name = "NexoLoadingScreen"
    ...
    for i = 1, 4 do
        ...
        line.BackgroundColor3 = Nexo.Theme.Text
    ...
    shipBody.BackgroundColor3 = Nexo.Theme.Text
    ...
    shipPoint.BackgroundColor3 = Nexo.Theme.Text
    ...
    for i, config in ipairs(trailConfigs) do
        ...
        trail.BackgroundColor3 = Nexo.Theme.Text
    ...
    for i, name in ipairs(phaseNames) do
        ...
        indicator.Text = "○"
        indicator.TextColor3 = Nexo.Theme.Pending
        ...
        label.Text = name
        label.TextColor3 = Nexo.Theme.Pending
    ...
    local function setPhase(num)
        ...
        for i = 1, 5 do
            ...
            if i < num then
                p.indicator.Text = "●"
                TweenService:Create(p.indicator, TweenInfo.new(0.2), {TextColor3 = Nexo.Theme.Success, TextTransparency = 0}):Play()
                TweenService:Create(p.label, TweenInfo.new(0.2), {TextColor3 = Nexo.Theme.Success}):Play()
            elseif i == num then
                p.indicator.Text = "●"
                p.indicator.TextTransparency = 0
                TweenService:Create(p.indicator, TweenInfo.new(0.2), {TextColor3 = Nexo.Theme.Accent}):Play()
                TweenService:Create(p.label, TweenInfo.new(0.2), {TextColor3 = Nexo.Theme.Text}):Play()
                currentPhase = num
                ...
                p.indicator.Text = "○"
                p.indicator.TextColor3 = Nexo.Theme.Pending
                p.label.TextColor3 = Nexo.Theme.Pending
            end
        end
    end
...
function Nexo:Notify(title, message, duration, iconType)
    ...
    frame.BackgroundColor3 = Nexo.Theme.Header
    frame.BorderSizePixel = 0
    frame.Parent = notifGui
    Instance.new("UICorner", frame).CornerRadius = UDim.new(0, 4)

    local stroke = Instance.new("UIStroke", frame)
    stroke.Color = Nexo.Theme.Accent
    stroke.Thickness = 1
    ...
    progressBar.BackgroundColor3 = Nexo.Theme.Accent
    ...
    local iconMap = {
        success = {"check", Nexo.Theme.Success}, error = {"alert", Nexo.Theme.Error},
        warning = {"alert", Nexo.Theme.Warning}, shield = {"shield", Nexo.Theme.Accent},
        info = {"shield", Nexo.Theme.Accent}, key = {"key", Nexo.Theme.Accent},
        copy = {"copy", Nexo.Theme.Success}, discord = {"discord", Nexo.Theme.Discord},
        close = {"close", Nexo.Theme.Error}
    }
    ...
    if iconMap[iconType] then
        ...
        icon.ImageColor3 = Nexo.Theme.Text
    end
    ...
    titleLabel.TextColor3 = Nexo.Theme.Text
    ...
    messageLabel.TextColor3 = Nexo.Theme.TextDim
...
local function CreateChangelogPanel(parent, windowWidth, panelHeight, panelWidth, mainFrame, gap)
    ...
    panel.BackgroundColor3 = Nexo.Theme.Background
    ...
    panelStroke.Color = Nexo.Theme.Accent
    panelStroke.Thickness = 2
    panelStroke.Transparency = 1

    local panelHeader = Instance.new("Frame")
    panelHeader.Size = UDim2.new(1, 0, 0, 50)
    panelHeader.BackgroundColor3 = Nexo.Theme.Header
    ...
    panelHeaderFix.BackgroundColor3 = Nexo.Theme.Header
    ...
    panelHeaderLine.BackgroundColor3 = Nexo.Theme.Accent
    ...
    panelHeaderIcon.ImageColor3 = Nexo.Theme.Accent
    ...
    panelTitle.TextColor3 = Nexo.Theme.Text
    ...
    panelClose.ImageColor3 = Nexo.Theme.TextDim
    panelClose.ScaleType = Enum.ScaleType.Fit
    panelClose.Parent = panelHeader
    panelClose.MouseEnter:Connect(function() TweenService:Create(panelClose, TweenInfo.new(0.15), {ImageColor3 = Nexo.Theme.Error}):Play() end)
    panelClose.MouseLeave:Connect(function() TweenService:Create(panelClose, TweenInfo.new(0.15), {ImageColor3 = Nexo.Theme.TextDim}):Play() end)
    ...
    scrollFrame.ScrollBarImageColor3 = Nexo.Theme.Accent
    ...
    for i, update in ipairs(Nexo.Changelog) do
        ...
        versionLabel.TextColor3 = Nexo.Theme.Accent
        ...
        for j, change in ipairs(update.Changes) do
            ...
            changeLabel.TextColor3 = Nexo.Theme.TextDim
        ...
        if i < #Nexo.Changelog then
            ...
            div.BackgroundColor3 = Nexo.Theme.Divider
...
local function CreateUserInfoPanel(parent, windowWidth, panelHeight, panelWidth, mainFrame, gap, startOpen)
    ...
    avatarGlow.BackgroundColor3 = Nexo.Theme.Accent
    ...
    avatarGlowStroke.Color = Nexo.Theme.Accent
    ...
    avatarContainer.BackgroundColor3 = Nexo.Theme.Input
    ...
    welcomeLabel.TextColor3 = Nexo.Theme.Text
    ...
    divider1.BackgroundColor3 = Nexo.Theme.Divider
    ...
    executorTitle.TextColor3 = Nexo.Theme.TextDim
    ...
    executorValue.TextColor3 = Nexo.Theme.Accent
    ...
    deviceTitle.TextColor3 = Nexo.Theme.TextDim
    ...
    deviceValue.TextColor3 = Nexo.Theme.Accent
    ...
    divider2.BackgroundColor3 = Nexo.Theme.Divider
    ...
    hwidTitle.Text = "HWID"
    hwidTitle.TextColor3 = Nexo.Theme.TextDim
    ...
    hwidValue.TextColor3 = Nexo.Theme.TextDim
    ...
    copyBtn.ImageColor3 = Nexo.Theme.TextDim
    copyBtn.ScaleType = Enum.ScaleType.Fit
    copyBtn.Parent = hwidContainer
    copyBtn.MouseEnter:Connect(function() TweenService:Create(copyBtn, TweenInfo.new(0.15), {ImageColor3 = Nexo.Theme.Accent}):Play() end)
    copyBtn.MouseLeave:Connect(function() TweenService:Create(copyBtn, TweenInfo.new(0.15), {ImageColor3 = Nexo.Theme.TextDim}):Play() end)
    copyBtn.MouseButton1Click:Connect(function()
        pcall(function() setclipboard(fullHWID) end)
        TweenService:Create(copyBtn, TweenInfo.new(0.1), {ImageColor3 = Nexo.Theme.Success}):Play()
        task.delay(0.3, function() TweenService:Create(copyBtn, TweenInfo.new(0.15), {ImageColor3 = Nexo.Theme.TextDim}):Play() end)
        Nexo:Notify("Copied", "HWID copied to clipboard", 2, "copy")
    end)
    ...
    divider3.BackgroundColor3 = Nexo.Theme.Divider
    ...
    clockIcon.ImageColor3 = Nexo.Theme.Accent
    ...
    clockTimeLabel.TextColor3 = Nexo.Theme.Accent
    ...
    clockDateLabel.TextColor3 = Nexo.Theme.TextDim
...
local function handleKeylessSkip()
    ...
    getgenv().NexoLoaded = false
    Nexo:Notify("Access Granted", "Keyless access approved!", 3, "success")
    task.wait(0.3)
...
local function BuildCenteredUI(windowWidth, windowHeight, panelHeight, userPanelWidth, changelogPanelWidth, gap, buildContent)
    ...
    mainFrame.BackgroundColor3 = Nexo.Theme.Background
    mainFrame.BorderSizePixel = 0
    mainFrame.Parent = container
    Instance.new("UICorner", mainFrame).CornerRadius = UDim.new(0, 4)

    local mainStroke = Instance.new("UIStroke", mainFrame)
    mainStroke.Color = Nexo.Theme.Accent
...
local function BuildKeylessUI()
    local oldGui = hui:FindFirstChild("NexoKeylessSystem")
    if oldGui then oldGui:Destroy() end
    local oldGui2 = hui:FindFirstChild("NexoKeySystem")
    ...
    header.BackgroundColor3 = Nexo.Theme.Header
    ...
    headerFix.BackgroundColor3 = Nexo.Theme.Header
    ...
    headerLine.BackgroundColor3 = Nexo.Theme.Accent
    ...
    logo.ImageColor3 = Nexo.Theme.Text
    ...
    title.Text = Nexo.Appearance.Title
    title.TextColor3 = Nexo.Theme.Text
    ...
    closeBtn.ImageColor3 = Nexo.Theme.TextDim
    closeBtn.ScaleType = Enum.ScaleType.Fit
    closeBtn.Parent = header
    closeBtn.MouseEnter:Connect(function() TweenService:Create(closeBtn, TweenInfo.new(0.15), {ImageColor3 = Nexo.Theme.Error}):Play() end)
    closeBtn.MouseLeave:Connect(function() TweenService:Create(closeBtn, TweenInfo.new(0.15), {ImageColor3 = Nexo.Theme.TextDim}):Play() end)
    ...
    successBox.BackgroundColor3 = Nexo.Theme.Success
    ...
    successStroke.Color = Nexo.Theme.Success
    ...
    checkIcon.ImageColor3 = Nexo.Theme.Success
    ...
    successText.TextColor3 = Nexo.Theme.Success
    ...
    keylessText.TextColor3 = Nexo.Theme.TextDim
    ...
    divider.BackgroundColor3 = Nexo.Theme.Divider
    ...
    launchBtn.BackgroundColor3 = Nexo.Theme.Accent
    ...
    launchStroke.Color = Nexo.Theme.AccentHover
    ...
    launchIcon.ImageColor3 = Nexo.Theme.Text
    ...
    launchLabel.TextColor3 = Nexo.Theme.Text
    ...
    launchBtn.MouseEnter:Connect(function() TweenService:Create(launchBtn, TweenInfo.new(0.15), {BackgroundColor3 = Nexo.Theme.AccentHover}):Play() end)
    launchBtn.MouseLeave:Connect(function() TweenService:Create(launchBtn, TweenInfo.new(0.15), {BackgroundColor3 = Nexo.Theme.Accent}):Play() end)
    ...
    userBtn.BackgroundColor3 = Nexo.Theme.Background
    ...
    userIcon.ImageColor3 = Nexo.Theme.TextDim
    userIcon.ScaleType = Enum.ScaleType.Fit
    userIcon.Parent = userBtn
    userBtn.MouseEnter:Connect(function() TweenService:Create(userIcon, TweenInfo.new(0.15), {ImageColor3 = Nexo.Theme.Accent}):Play() end)
    userBtn.MouseLeave:Connect(function() TweenService:Create(userIcon, TweenInfo.new(0.15), {ImageColor3 = Nexo.Theme.TextDim}):Play() end)
    ...
    discordBtn.BackgroundColor3 = Nexo.Theme.Background
    ...
    discordIcon.ImageColor3 = Nexo.Theme.Discord
    discordIcon.ScaleType = Enum.ScaleType.Fit
    discordIcon.Parent = discordBtn
    discordBtn.MouseEnter:Connect(function() TweenService:Create(discordIcon, TweenInfo.new(0.15), {ImageColor3 = Nexo.Theme.DiscordHover}):Play() end)
    discordBtn.MouseLeave:Connect(function() TweenService:Create(discordIcon, TweenInfo.new(0.15), {ImageColor3 = Nexo.Theme.Discord}):Play() end)
    ...
    changelogBtn.BackgroundColor3 = Nexo.Theme.Background
    ...
    changelogIcon.ImageColor3 = Nexo.Theme.TextDim
    changelogIcon.ScaleType = Enum.ScaleType.Fit
    changelogIcon.Parent = changelogBtn
    changelogBtn.MouseEnter:Connect(function() TweenService:Create(changelogIcon, TweenInfo.new(0.15), {ImageColor3 = Nexo.Theme.Text}):Play() end)
    changelogBtn.MouseLeave:Connect(function() TweenService:Create(changelogIcon, TweenInfo.new(0.15), {ImageColor3 = Nexo.Theme.TextDim}):Play() end)

    if #Nexo.Changelog == 0 then
    ...
    closeBtn.MouseButton1Click:Connect(function()
        Nexo:Notify("Goodbye", "See you next time!", 2, "close")
        ...
        if Nexo.Callbacks.OnClose then Nexo.Callbacks.OnClose() end
    end)

    launchBtn.MouseButton1Click:Connect(function()
        Nexo:Notify("Launching", "Script loaded successfully!", 2, "success")
        getgenv().SCRIPT_KEY = "KEYLESS"
        getgenv().NexoLoaded = false
        closeDoorsThenExit(function()
            disableBlur()
            ...
            TweenService:Create(mainStroke, TweenInfo.new(0.3), {Transparency = 1}):Play()
            task.wait(0.4) gui:Destroy()
            if not Internal.IsNxuziMode and Nexo.Callbacks.OnSuccess then Nexo.Callbacks.OnSuccess() end
        end)
    end)

    discordBtn.MouseButton1Click:Connect(function()
        if Nexo.Links.Discord ~= "" then
            Nexo:Notify("Discord", "Invite link copied!", 2, "discord")
            pcall(function() setclipboard(Nexo.Links.Discord) end)
        end
    end)
    ...
    doors.open(function()
        ...
        if #Nexo.Changelog > 0 then task.wait(0.3) ui.toggleCL(changelogIcon) end
    end)
end

local function BuildKeyUI()
    local oldGui = hui:FindFirstChild("NexoKeySystem")
    if oldGui then oldGui:Destroy() end
    local oldGui2 = hui:FindFirstChild("NexoKeylessSystem")
    if oldGui2 then oldGui2:Destroy() end
    ...
    local logo = Instance.new("ImageLabel")
    logo.Size = Nexo.Appearance.IconSize
    logo.Position = UDim2.new(0, padding, 0.5, 0)
    logo.AnchorPoint = Vector2.new(0, 0.5)
    logo.BackgroundTransparency = 1
    logo.Image = getLogoIcon()
    ...
    titleLabel.Position = UDim2.new(0, padding + Nexo.Appearance.IconSize.X.Offset + 10, 0, 0)
    titleLabel.BackgroundTransparency = 1
    titleLabel.Text = Nexo.Appearance.Title
    titleLabel.TextColor3 = Nexo.Theme.Text
    ...
    statusFrame.BackgroundColor3 = Nexo.Theme.Input
    ...
    statusStroke.Color = Nexo.Theme.Accent
    ...
    statusIcon.ImageColor3 = Nexo.Theme.StatusIdle
    ...
    statusLabel.Text = Nexo.Appearance.Subtitle
    statusLabel.TextColor3 = Nexo.Theme.StatusIdle
    ...
    inputFrame.BackgroundColor3 = Nexo.Theme.Input
    ...
    inputStroke.Color = Nexo.Theme.Accent
    ...
    textBox.Text = ""
    textBox.TextColor3 = Nexo.Theme.Text
    textBox.PlaceholderText = "Enter your key..."
    textBox.PlaceholderColor3 = Nexo.Theme.TextDim
    ...
    dividerLine.BackgroundColor3 = Nexo.Theme.Divider
    ...
    local function createButton(text, iconKey, isPrimary, yPos)
        ...
        btn.BackgroundColor3 = isPrimary and Nexo.Theme.Accent or Nexo.Theme.Input
        ...
        btnStroke.Color = isPrimary and Nexo.Theme.AccentHover or Nexo.Theme.Accent
        ...
        iconImg.ImageColor3 = Nexo.Theme.Text
        ...
        label.BackgroundTransparency = 1
        label.Text = text
        label.TextColor3 = Nexo.Theme.Text
        ...
        local hoverColor = isPrimary and Nexo.Theme.AccentHover or Nexo.Theme.Accent
    ...
    if showShop then
        ...
        shopDivider.BackgroundColor3 = Nexo.Theme.Accent
        ...
        shopFrame.BackgroundColor3 = Nexo.Theme.Header
        ...
        shopTopFix.BackgroundColor3 = Nexo.Theme.Header
        ...
        shopIconWrapper.BackgroundColor3 = Nexo.Theme.Accent
        ...
        shopIconStroke.Color = Nexo.Theme.Accent
        ...
        shopIconImg.ImageColor3 = Nexo.Theme.Text
        ...
        shopTitle.Text = Nexo.Shop.Title
        shopTitle.TextColor3 = Nexo.Theme.Text
        ...
        shopSubtitle.Text = Nexo.Shop.Subtitle
        shopSubtitle.TextColor3 = Nexo.Theme.TextDim
        ...
        buyBtn.BackgroundColor3 = Nexo.Theme.Accent
        ...
        buyBtnStroke.Color = Nexo.Theme.AccentHover
        ...
        buyIcon.ImageColor3 = Nexo.Theme.Text
        ...
        buyLabel.Text = Nexo.Shop.ButtonText
        buyLabel.TextColor3 = Nexo.Theme.Text
        buyLabel.TextSize = mobile and 11 or 12
        buyLabel.Font = Enum.Font.ArimoBold
        buyLabel.LayoutOrder = 2
        buyLabel.Parent = buyContent

        buyBtn.MouseEnter:Connect(function() TweenService:Create(buyBtn, TweenInfo.new(0.15), {BackgroundColor3 = Nexo.Theme.AccentHover}):Play() end)
        buyBtn.MouseLeave:Connect(function() TweenService:Create(buyBtn, TweenInfo.new(0.15), {BackgroundColor3 = Nexo.Theme.Accent}):Play() end)
        buyBtn.MouseButton1Click:Connect(function()
            if Nexo.Shop.Link ~= "" then
                pcall(function() setclipboard(Nexo.Shop.Link) end)
                Nexo:Notify("Shop", "Shop link copied to clipboard!", 2, "copy")
            end
        end)
    end
    ...
    local function setStatus(state, customText)
        ...
        local color, icon, text = Nexo.Theme.StatusIdle, getIcon("lock"), customText or "No key detected"
        if state == "verifying" then
            color, icon, text = Nexo.Theme.Accent, getIcon("loading"), "Verifying key"
        ...
        elseif state == "success" then color, icon, text = Nexo.Theme.Success, getIcon("check"), customText or "Access Granted"
        elseif state == "error" then color, icon, text = Nexo.Theme.Error, getIcon("alert"), customText or "Invalid Key" end
    ...
    local function handleRedeem()
        ...
        if key == "" then Nexo:Notify("Error", "Please enter your key", 3, "warning") return end
        ...
        if valid then
            saveKey(key) getgenv().SCRIPT_KEY = key getgenv().NexoLoaded = false
            setStatus("success") Nexo:Notify("Success", "Key validated successfully!", 2, "success") task.wait(1)
            closeDoorsThenExit(function()
                disableBlur()
                TweenService:Create(container, TweenInfo.new(0.4, Enum.EasingStyle.Quart), {Position = UDim2.new(0.5, 0, -0.5, 0)}):Play()
                TweenService:Create(mainFrame, TweenInfo.new(0.3), {BackgroundTransparency = 1}):Play()
            ...
            setStatus("error", errorMsg) Nexo:Notify("Invalid", errorMsg, 4, "error")
            if Nexo.Callbacks.OnFail then Nexo.Callbacks.OnFail(errorMsg) end
        end
    end

    redeemBtn.MouseButton1Click:Connect(handleRedeem)
    acquireBtn.MouseButton1Click:Connect(function()
        if Nexo.Links.GetKey ~= "" then Nexo:Notify("Copied", "Key link copied!", 3, "copy") pcall(function() setclipboard(Nexo.Links.GetKey) end)
        else Nexo:Notify("Error", "No key link set", 3, "warning") end
    end)
    discordBtn.MouseButton1Click:Connect(function()
        if Nexo.Links.Discord ~= "" then Nexo:Notify("Discord", "Invite link copied!", 2, "discord") pcall(function() setclipboard(Nexo.Links.Discord) end) end
    end)
...
function Nexo:Launch()
    Internal.IsNxuziMode = false
    Internal.ValidateFunction = Nexo.Callbacks.OnVerify
...
function Nexo:LaunchNxuzi(config)
    ...
    getgenv().NexoClosed = false
    EnsureIconsReady(function()
        local success, Nxuzi = pcall(function() return loadstring(game:HttpGet("https://jnkie.com/sdk/library.lua"))() end)
...
function Nexo:GetSavedKey() return loadKey() end
function Nexo:ClearSavedKey() return clearKey() end
